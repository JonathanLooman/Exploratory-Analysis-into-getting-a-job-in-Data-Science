...
def count_languages(df1, question_dict, questionnNumber = 'Q7'):
    ...
    for x in df1.columns:
        if x[0:2] == questionnNumber:
            internal_dict[df1.loc[0,x].split(" - ")[-1]] = df1[x].count()
        
    
    return internal_dict

def percent_dictionary(df1, questionnNumber = 'Q7'):
...
def breakdown_percent(df1,questionnNumber,question_dict ):
    
    '''Main class to call'''
    if is_multiple(df1 = df1, questionnNumber=questionnNumber ) == True:
        print('works')
        dict1 = percent_dictionary(questionnNumber = questionnNumber, df1 = df1 )
        plot_dict(dict1)
